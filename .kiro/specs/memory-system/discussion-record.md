...
**分離式服務架構**：
- **模型服務** = 眼睛：獨立運行VLM持續觀察，將視覺世界數字化
- **後端服務** = 比對引擎 + 白板：獨立運行State Tracker + RAG，智能匹配並記錄狀態
- **前端服務** = 用戶界面：獨立運行用戶交互和查詢處理

**設計哲學**：
- 服務分離：三個獨立服務各自負責專門功能，通過通信協同工作
- 潛意識狀態感知：模型服務在用戶無感知情況下持續觀察，後端服務持續理解狀態
- 即時響應能力：前端查詢時後端服務毫秒級回應，無需重新計算
- 記憶體可控：後端服務通過滑動窗格確保資源使用恆定

### 🔄 雙循環架構設計

#### 第一循環：跨服務「潛意識」狀態感知（持續運行）
```mermaid
graph TB
    subgraph "模型服務"
        A[VLM眼睛觀察] --> B[視覺世界數字化]
...
- step_id: 2
  ...
  completion_indicators: ["咖啡粉粗細均勻", "研磨聲停止"]
