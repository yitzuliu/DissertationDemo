...
## 階段2：State Tracker 比對引擎 **[0/4 完成]**

- [ ] 2.1 實現VLM觀察器（系統眼睛）
  - 創建無狀態的VLM觀察接口，接受任意格式的VLM輸出
  - 實現持續螢幕捕獲機制（每1~10秒）
  - 添加VLM輸出的清理和標準化處理（處理亂碼、空輸出、異常格式）（Demo時確認）
  - 實現VLM失敗時的跳過機制和錯誤日誌
  - 創建VLM觀察的模擬接口（用於測試不穩定輸出）
  - _展示價值: 持續視覺感知 + 容錯能力_

- [ ] 2.2 建立State Tracker核心比對引擎
  - 實現VLM觀察與RAG知識的向量匹配（處理不完整或錯誤的VLM輸入）
